...
#SBATCH --output casimodo_DHFR_%j.log
#SBATCH --job-name=CASIMODO_DHFR
#SBATCH --partition=qAVX2 
#SBATCH --nodes=1               # of nodes
#SBATCH --ntasks-per-node=1    # of MPI tasks/node on CPU (Might depend on the number of GPUs)
#SBATCH --cpus-per-task=1       # of OMP threads/task on CPU (default =  1)
...
output_directory=results_CASIMODO_DHFR_${T}K

# topology file in a format readable by MDAnalysis
topol_file=REMD_simulations_WT_DHFR_nowater/REMD_DHFR_WT_nowater_center_fit0.gro

# Trajectory file in a format readable by MDAnalysis, centered and fitted
trj_file=REMD_simulations_WT_DHFR_nowater/REMD_DHFR_WT_nowater_center_fit_${T}K.xtc

##############################################
#  POSITION of CASIMODO FUNCTIONS (Crucial)  #
##############################################
position_CASIMODO=/home/pguenon/home_pguenon/CASIMODO/CASIMODO_utils/  # Position of the CASIMODO_utils folder relative to this script. Default is "CASIMODO_utils/" if the directory is in the same location as this script. Adjust if necessary.
